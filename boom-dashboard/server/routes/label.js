...
router.get('/', authMiddleware, async (req, res) => {
  try { res.json({ success: true, data: masked(await readRow()) }); }
...
router.put('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (!sets.length) return res.json({ success: true, data: masked(await readRow()) });
    vals.push(req.user.id);
    await pool.query(`UPDATE label_settings SET ${sets.join(', ')}, updated_at = NOW(), updated_by = $${vals.length} WHERE id = 1`, vals);
    res.json({ success: true, data: masked(await readRow()) });
...
router.get('/remittance', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { ...masked(r), ein, bank_account_number: account } });
