#!/usr/bin/env node
/**
 * Settings in two halves and the People directory, under jsdom. See settings-dom-entry.jsx.
 *
 *     mkdir -p /tmp/domtest && cd /tmp/domtest && npm init -y && npm i jsdom
 *     cd client && npm run settings-dom
 *
 * Fully stubbed. Scenarios: admin · user · people.
 */
...
execFileSync('npx', ['vite', 'build', '-c', 'scripts/mywork-dom.vite.config.mjs'], {
  cwd: CLIENT, env: { ...process.env, ENTRY: 'scripts/settings-dom-entry.jsx', API_STUB: 'scripts/settings-api-stub.js', AUTH_STUB: 'scripts/home-dom-auth-stub.js' }, stdio: ['ignore', 'ignore', 'pipe'],
})
const scenarios = process.env.SET_SCENARIO ? [process.env.SET_SCENARIO] : ['admin', 'user', 'people']
let all = ''
for (const sc of scenarios) {
  let log = ''
  try {
    log = execFileSync('node', ['scripts/mywork-dom-check.mjs', '.domsmoke/out/settings-dom-entry.js'], {
      cwd: CLIENT, env: { ...process.env, JSDOM_PATH, WAIT_FOR_DONE: '1', WAIT_MS: '30000', PAGE_URL: 'http://localhost/', SET_SCENARIO: sc },
