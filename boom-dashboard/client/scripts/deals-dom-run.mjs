#!/usr/bin/env node
/**
 * Does the deal pipeline's second pass draw and act? Board with folded Signed /
 * Passed, cards with owner · days · follow-up · last touch, URL filters, the
 * drawer (timeline, checklist, owner), the Passed modal, the list, the report.
 *
 *     mkdir -p /tmp/domtest && cd /tmp/domtest && npm init -y && npm i jsdom
 *     cd client && npm run deals-dom
 *
 * Fully stubbed (scripts/deals-api-stub.js). Scenarios: full · empty · url.
 */
...
execFileSync('npx', ['vite', 'build', '-c', 'scripts/mywork-dom.vite.config.mjs'], {
  cwd: CLIENT,
  env: { ...process.env, ENTRY: 'scripts/deals-dom-entry.jsx', API_STUB: 'scripts/deals-api-stub.js', AUTH_STUB: 'scripts/flags-dom-auth-stub.js' },
  stdio: ['ignore', 'ignore', 'pipe'],
})

const scenarios = process.env.DEALS_SCENARIO ? [process.env.DEALS_SCENARIO] : ['full', 'empty', 'url']
let all = ''
for (const sc of scenarios) {
  let log = ''
  try {
    log = execFileSync('node', ['scripts/mywork-dom-check.mjs', '.domsmoke/out/deals-dom-entry.js'], {
      cwd: CLIENT,
      env: { ...process.env, JSDOM_PATH, WAIT_FOR_DONE: '1', WAIT_MS: '45000', PAGE_URL: 'http://localhost/deals', DEALS_SCENARIO: sc },
