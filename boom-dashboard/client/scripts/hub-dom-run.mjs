#!/usr/bin/env node
/**
 * The artist profile as a hub, driven under jsdom. See hub-dom-entry.jsx.
 *
 *     mkdir -p /tmp/domtest && cd /tmp/domtest && npm init -y && npm i jsdom
 *     cd client && npm run hub-dom
 *
 * Fully stubbed. Scenarios: full · nocampaign · gated.
 */
...
execFileSync('npx', ['vite', 'build', '-c', 'scripts/mywork-dom.vite.config.mjs'], {
  cwd: CLIENT, env: { ...process.env, ENTRY: 'scripts/hub-dom-entry.jsx', API_STUB: 'scripts/hub-api-stub.js', AUTH_STUB: 'scripts/home-dom-auth-stub.js' }, stdio: ['ignore', 'ignore', 'pipe'],
})
const scenarios = process.env.HUB_SCENARIO ? [process.env.HUB_SCENARIO] : ['full', 'nocampaign', 'gated']
let all = ''
for (const sc of scenarios) {
  let log = ''
  try {
    log = execFileSync('node', ['scripts/mywork-dom-check.mjs', '.domsmoke/out/hub-dom-entry.js'], {
      cwd: CLIENT, env: { ...process.env, JSDOM_PATH, WAIT_FOR_DONE: '1', WAIT_MS: '30000', PAGE_URL: 'http://localhost/', HUB_SCENARIO: sc },
