#!/usr/bin/env node
/**
 * The team calendar, driven under jsdom. See calendar-dom-entry.jsx.
 *
 *     mkdir -p /tmp/domtest && cd /tmp/domtest && npm init -y && npm i jsdom
 *     cd client && npm run calendar-dom
 *
 * Fully stubbed. Scenarios: full · gated · empty.
 */
...
execFileSync('npx', ['vite', 'build', '-c', 'scripts/mywork-dom.vite.config.mjs'], {
  cwd: CLIENT, env: { ...process.env, ENTRY: 'scripts/calendar-dom-entry.jsx', API_STUB: 'scripts/calendar-api-stub.js' }, stdio: ['ignore', 'ignore', 'pipe'],
})
const scenarios = process.env.CAL_SCENARIO ? [process.env.CAL_SCENARIO] : ['full', 'gated', 'empty']
let all = ''
for (const sc of scenarios) {
  let log = ''
  try {
    log = execFileSync('node', ['scripts/mywork-dom-check.mjs', '.domsmoke/out/calendar-dom-entry.js'], {
      cwd: CLIENT, env: { ...process.env, JSDOM_PATH, WAIT_FOR_DONE: '1', WAIT_MS: '30000', PAGE_URL: 'http://localhost/', CAL_SCENARIO: sc },
